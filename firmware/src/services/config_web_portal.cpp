...
#include "../ui_runtime/dynamic_ui.h"
#include "../ui_runtime/ui_package_store.h"
#include "../ui_runtime/bmsui_protocol.h"
...
File uiUploadFile;
bool uiUploadStarted = false;
bool uiUploadSucceeded = false;
bool uiUploadRejected = false;
size_t uiUploadBytes = 0U;
char uiUploadMessage[128] = {};

enum class UiCommitState : uint8_t {
  Idle,
  Pending,
  Running,
  Success,
...
UiCommitState uiCommitState = UiCommitState::Idle;
uint32_t uiCommitNotBefore = 0;
size_t uiCommitBytes = 0U;
char uiCommitMessage[128] = "idle";
...
<header class="top"><div><div class="brand">无用脑洞研究所</div><div class="sub">保护板配置中心</div></div><span class="version">)HTML" BMS_DISPLAY_FIRMWARE_VERSION R"HTML(</span></header>
...
void handleHealth() {
  ...
  String text;
  text.reserve(240);
  text = F("BMS WEB OK\nVersion=" BMS_DISPLAY_FIRMWARE_VERSION "\nDeveloper=无用脑洞研究所\nIP=192.168.4.1\nSSID=");
  text += apName;
  text += F("\nStations=");
...
const char *uiCommitStateName() {
  switch (uiCommitState) {
    case UiCommitState::Pending: return "pending";
    case UiCommitState::Running: return "running";
    case UiCommitState::Success: return "success";
    case UiCommitState::Failed: return "failed";
    case UiCommitState::Idle:
    default: return "idle";
  }
}

void resetUiUploadState() {
  if (uiUploadFile) uiUploadFile.close();
  uiUploadStarted = false;
  uiUploadSucceeded = false;
  uiUploadRejected = false;
  uiUploadBytes = 0U;
  uiUploadMessage[0] = '\0';
  if (uiCommitState != UiCommitState::Running && uiCommitState != UiCommitState::Pending) {
    uiCommitState = UiCommitState::Idle;
    uiCommitBytes = 0U;
    uiCommitNotBefore = 0U;
    snprintf(uiCommitMessage, sizeof(uiCommitMessage), "idle");
  }
}

void rejectUiUpload(const char *message) {
  uiUploadRejected = true;
  uiUploadSucceeded = false;
  snprintf(uiUploadMessage,
           sizeof(uiUploadMessage),
           "%s",
           message == nullptr ? "UI upload rejected" : message);
  if (uiUploadFile) uiUploadFile.close();
  UiPackageStore::discardStaging();
}

void handleUiInfo() {
  if (!requireAuthentication()) return;

  const bool present = UiPackageStore::hasCustomPackage();
  const bool valid = present && DynamicUi::packageValid();
  const bool stagingPresent = UiPackageStore::stagingExists();

  String payload;
  payload.reserve(800);
  payload = F("{\"ok\":true,\"protocolVersion\":");
  payload += String(BmsUi::ProtocolVersion);
  payload += F(",\"width\":");
  payload += String(AppConfig::Display::Width);
  payload += F(",\"height\":");
  payload += String(AppConfig::Display::Height);
  const size_t psramSize = ESP.getPsramSize();
  const size_t runtimeAssetLimit = psramSize >= (1024U * 1024U)
                                       ? BmsUi::MaxRuntimeAssetBytes
                                       : BmsUi::BaseRuntimeAssetBytes;
  payload += F(",\"psramSize\":");
  payload += String(static_cast<unsigned long>(psramSize));
  payload += F(",\"psramFree\":");
  payload += String(static_cast<unsigned long>(heap_caps_get_free_size(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)));
  payload += F(",\"psramLargest\":");
  payload += String(static_cast<unsigned long>(heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)));
  payload += F(",\"internalFree\":");
  payload += String(static_cast<unsigned long>(heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)));
  payload += F(",\"internalLargest\":");
  payload += String(static_cast<unsigned long>(heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT)));
  payload += F(",\"runtimeAssetLimit\":");
  payload += String(static_cast<unsigned long>(runtimeAssetLimit));
  payload += F(",\"present\":");
  payload += present ? "true" : "false";
  payload += F(",\"valid\":");
  payload += valid ? "true" : "false";
  payload += F(",\"loaded\":");
  payload += DynamicUi::active() ? "true" : "false";
  payload += F(",\"size\":");
  payload += String(UiPackageStore::currentSize());
  payload += F(",\"fsMounted\":");
  payload += UiPackageStore::mounted() ? "true" : "false";
  payload += F(",\"fsTotal\":");
  payload += String(UiPackageStore::totalBytes());
  payload += F(",\"fsUsed\":");
  payload += String(UiPackageStore::usedBytes());
  payload += F(",\"stagingPresent\":");
  payload += stagingPresent ? "true" : "false";
  payload += F(",\"stagingSize\":");
  payload += String(UiPackageStore::stagingSize());
  payload += F(",\"stagingValid\":false,\"stagingMessage\":\"");
  payload += stagingPresent ? "pending (not parsed by status endpoint)" : "none";
  payload += F("\",\"backupPresent\":");
  payload += UiPackageStore::backupExists() ? "true" : "false";
  payload += F(",\"backupSize\":");
  payload += String(UiPackageStore::backupSize());
  payload += F(",\"rejectedPresent\":");
  payload += UiPackageStore::rejectedExists() ? "true" : "false";
  payload += F(",\"rejectedSize\":");
  payload += String(UiPackageStore::rejectedSize());
  payload += F(",\"storageMessage\":\"");
  payload += jsonEscape(UiPackageStore::lastStorageError());
  payload += F("\",\"title\":\"");
  payload += jsonEscape(DynamicUi::title());
  payload += F("\",\"runtimeMessage\":\"");
  payload += jsonEscape(DynamicUi::lastError());
  payload += F("\",\"validationMessage\":\"");
  if (!present) payload += F("factory UI");
  else if (valid) payload += F("validated during boot");
  else payload += F("not validated/loaded during this boot");
  payload += F("\"}");
  sendJson(200, payload);
}

void handleUiDownload() {
  if (!requireAuthentication()) return;
  if (!UiPackageStore::hasCustomPackage()) {
    sendJson(404, "{\"ok\":false,\"message\":\"当前没有自定义UI\"}");
    return;
  }

  File file = UiPackageStore::openCurrent(FILE_READ);
  if (!file) {
    sendJson(500, "{\"ok\":false,\"message\":\"无法读取UI文件\"}");
    return;
  }

  noteAccess();
  addCommonHeaders();
  server.sendHeader("Content-Disposition", "attachment; filename=\"current.bmsui\"");
...
void handleUiUploadData() {
  HTTPUpload &upload = server.upload();

  switch (upload.status) {
    case UPLOAD_FILE_START:
      if (uiCommitState == UiCommitState::Pending || uiCommitState == UiCommitState::Running) {
        uiUploadStarted = true;
        rejectUiUpload("previous UI commit is still in progress");
        return;
      }
      resetUiUploadState();
      uiUploadStarted = true;
      if (!isAuthenticated()) {
        rejectUiUpload("authentication required");
        return;
      }
      if (!UiPackageStore::beginStaging(uiUploadFile)) {
        rejectUiUpload("unable to create SPIFFS staging file");
        return;
      }
      break;

    case UPLOAD_FILE_WRITE:
      if (!uiUploadStarted || uiUploadRejected) return;
      if (uiUploadBytes + upload.currentSize > BmsUi::MaxPackageBytes) {
        rejectUiUpload("UI package exceeds firmware size limit");
        return;
      }
      if (!uiUploadFile ||
          uiUploadFile.write(upload.buf, upload.currentSize) != upload.currentSize) {
        rejectUiUpload("SPIFFS write failed");
        return;
      }
      uiUploadBytes += upload.currentSize;
#if defined(ARDUINO)
      delay(0);
#endif
      break;
    ...
    case UPLOAD_FILE_END:
      ...
      if (uiUploadBytes == 0U) {
        rejectUiUpload("uploaded UI file is empty");
        return;
      }

      uiUploadSucceeded = true;
      snprintf(uiUploadMessage, sizeof(uiUploadMessage), "staged");
      uiCommitBytes = uiUploadBytes;
      uiCommitState = UiCommitState::Pending;
      uiCommitNotBefore = TimeUtils::deadlineAfter(
          millis(), AppConfig::WebConfig::UiCommitStartDelayMs);
      snprintf(uiCommitMessage, sizeof(uiCommitMessage), "pending");
      break;

    case UPLOAD_FILE_ABORTED:
      rejectUiUpload("upload aborted");
      break;
...
void handleUiUploadComplete() {
  if (!requireAuthentication()) return;
  if (!uiUploadStarted) {
    sendJson(400, "{\"ok\":false,\"message\":\"没有收到UI文件\"}");
    return;
  }

  if (!uiUploadSucceeded) {
    String payload = F("{\"ok\":false,\"message\":\"");
    payload += jsonEscape(uiUploadMessage[0] == '\0' ? "UI upload failed" : uiUploadMessage);
    payload += F("\"}");
    sendJson(400, payload);
    return;
  }

  String payload = F("{\"ok\":true,\"accepted\":true,\"persisted\":false,\"rebooting\":false,\"commitState\":\"pending\",\"size\":");
  payload += String(uiUploadBytes);
  payload += F(",\"stagingPresent\":");
  payload += UiPackageStore::stagingExists() ? "true" : "false";
  payload += F("}");
  sendJson(202, payload);
}

void handleUiCommitStatus() {
  if (!requireAuthentication()) return;
  String payload;
  payload.reserve(512);
  payload = F("{\"ok\":true,\"state\":\"");
  payload += uiCommitStateName();
  payload += F("\",\"message\":\"");
  payload += jsonEscape(uiCommitMessage);
  payload += F("\",\"size\":");
  payload += String(uiCommitBytes);
  payload += F(",\"currentSize\":");
  payload += String(UiPackageStore::currentSize());
  payload += F(",\"present\":");
  payload += UiPackageStore::hasCustomPackage() ? "true" : "false";
  payload += F(",\"stagingPresent\":");
  payload += UiPackageStore::stagingExists() ? "true" : "false";
  payload += F(",\"rebooting\":");
  payload += (uiCommitState == UiCommitState::Success && rebootAt != 0U) ? "true" : "false";
  payload += F("}");
  sendJson(200, payload);
}

void serviceDeferredUiCommit(uint32_t now) {
  if (uiCommitState != UiCommitState::Pending ||
      !TimeUtils::deadlineReached(now, uiCommitNotBefore)) {
    return;
  }

  uiCommitState = UiCommitState::Running;
  snprintf(uiCommitMessage, sizeof(uiCommitMessage), "validating");

  BmsUi::ParseResult result;
  if (!UiPackageStore::commitStaging(result)) {
    uiCommitState = UiCommitState::Failed;
    snprintf(uiCommitMessage,
             sizeof(uiCommitMessage),
             "line %u: %s",
             static_cast<unsigned>(result.line),
             result.message);
    DiagnosticLog::printf("Dynamic UI commit failed: %s\n", uiCommitMessage);
    return;
  }

  uiCommitState = UiCommitState::Success;
  snprintf(uiCommitMessage, sizeof(uiCommitMessage), "saved");
  DiagnosticLog::printf("Dynamic UI saved: %u bytes.\n",
                        static_cast<unsigned>(UiPackageStore::currentSize()));
  rebootAt = TimeUtils::deadlineAfter(
      millis(), AppConfig::WebConfig::UiCommitRestartDelayMs);
}

void handleUiRecover() {
  if (!requireAuthentication()) return;
  BmsUi::ParseResult result;
  if (!UiPackageStore::recoverPendingPackage(result) ||
      !UiPackageStore::hasCustomPackage()) {
    String payload = F("{\"ok\":false,\"message\":\"");
    payload += jsonEscape(result.message);
    payload += F("\",\"stagingPresent\":");
    payload += UiPackageStore::stagingExists() ? "true" : "false";
    payload += F(",\"stagingSize\":");
    payload += String(UiPackageStore::stagingSize());
  ...
  String payload = F("{\"ok\":true,\"recovered\":true,\"rebooting\":true,\"size\":");
  payload += String(UiPackageStore::currentSize());
  payload += F("}");
  sendJson(200, payload);
  rebootAt = TimeUtils::deadlineAfter(millis(), AppConfig::WebConfig::SaveRestartDelayMs);
}

void handleUiReset() {
  if (!requireAuthentication()) return;
  if (!UiPackageStore::resetCustomPackage()) {
    sendJson(500, "{\"ok\":false,\"message\":\"删除自定义UI失败\"}");
    return;
  }
  sendJson(200, "{\"ok\":true,\"rebooting\":true}");
  rebootAt = TimeUtils::deadlineAfter(millis(), AppConfig::WebConfig::SaveRestartDelayMs);
...
void configureRoutes() {
  ...
  server.on("/api/ui/info", HTTP_GET, handleUiInfo);
  server.on("/api/ui/download", HTTP_GET, handleUiDownload);
  server.on("/api/ui/upload", HTTP_POST, handleUiUploadComplete, handleUiUploadData);
  server.on("/api/ui/commit-status", HTTP_GET, handleUiCommitStatus);
  server.on("/api/ui/recover", HTTP_POST, handleUiRecover);
  server.on("/api/ui/reset", HTTP_POST, handleUiReset);
