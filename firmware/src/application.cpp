...
void handleLongPress() {
  ...
  DiagnosticLog::write(scanRequested
                           ? "10-second hold: configuration/UI portal requested; BLE pairing preserved.\n"
                           : "10-second hold: failed to request configuration/UI portal.\n");
...
void logStartupBanner() {
  ...
  DiagnosticLog::write("Build profile: PSRAM-AUTO-RUNTIME-3\n");
  const size_t earlyPsramSize = ESP.getPsramSize();
  const size_t earlyPsramTotal = heap_caps_get_total_size(MALLOC_CAP_SPIRAM);
  const size_t earlyPsramFree = heap_caps_get_free_size(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  const size_t earlyPsramLargest = heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  DiagnosticLog::printf(
      "EARLY PSRAM: Arduino=%u, heap-cap=%u, free=%u, largest=%u bytes.\n",
      static_cast<unsigned>(earlyPsramSize),
      static_cast<unsigned>(earlyPsramTotal),
      static_cast<unsigned>(earlyPsramFree),
      static_cast<unsigned>(earlyPsramLargest));
  ...
  DiagnosticLog::write(
      testMode
          ...
          : "Button: click=page, triple-click=screen, hold 10s=open config/UI portal\n");
  DiagnosticLog::write(
...
void loop() {
  ...
  if (dashboardTestModeActive()) {
    // In dashboard test mode the Web portal is the control path. Service it
    // before LVGL so a phone/PC that has joined the SoftAP can immediately
    // open 192.168.4.1 and upload a UI package.
    ...
    // Rendering a complete dynamic dashboard every 50 ms can starve the
    // synchronous WebServer on ESP32-S3. While a portal client is attached,
    // keep the last test frame stable and give HTTP/UI upload top priority.
    // Animation resumes automatically as soon as the client disconnects.
    if (!portalClient && DashboardTestMode::AnimateSoc &&
    ...
    if (DashboardTestMode::EnableAutoDisplayPowerTest) {
      DisplayPowerManager::loop(now, portalClient, true);
    }
