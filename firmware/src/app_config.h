...
#define BMS_DISPLAY_FIRMWARE_VERSION "v1.1.0"

namespace AppConfig {

// Firmware identity shown by serial logs, Web UI, and diagnostics.
namespace Firmware {
constexpr char Name[] = "Multi-BMS ESP32-S3 Display";
constexpr char Version[] = BMS_DISPLAY_FIRMWARE_VERSION;
...
// Parking dim/sleep thresholds. Currents use the normalized sign: charge +, discharge -.
...
// Active-low button timing. Three clicks toggle display; 10 s long press enters Web config.
namespace Button {
...
// BLE names, UUIDs, scan/reconnect timing, connection parameters, and RX buffering.
namespace Ble {
...
// Remaining-range model. km/Ah is user-adjustable through the Web portal.
namespace Range {
...
// SoftAP, login, startup/retry, and UI commit timing. Change the default password after setup.
namespace WebConfig {
...
constexpr uint32_t UiCommitRestartDelayMs = 3500;
constexpr uint32_t UiCommitStartDelayMs = 120;
constexpr uint32_t RescanRestartDelayMs = 900;
constexpr uint32_t RebootFlushDelayMs = 80;
}

// Parser bounds and sanity limits. These protect the runtime from malformed BMS frames.
namespace Protocol {
...
// Top-level startup, loop yield, restart, and reconnect delays.
namespace Runtime {
