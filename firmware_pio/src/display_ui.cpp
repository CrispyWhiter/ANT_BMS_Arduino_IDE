...
bool begin() {
  ...
  ui_init();
  const bool customUiLoaded = DynamicUi::begin();
  if (customUiLoaded) {
    lv_scr_load(DynamicUi::screen());
    if (objects.main != nullptr) lv_obj_del(objects.main);
    objects = objects_t{};
  } else {
    MainPage::begin();
  }
...
void updatePreview(const BmsData &data,
  ...
  DynamicUi::updatePreview(data, previewRangeKm, previewPowerW);
  CellPage::update(data);
}
...
void setConnected(bool connected) {
  ...
  DynamicUi::setConnected(connected);
}
...
void showPage(Page page) {
  ...
  switch (page) {
    case Page::Main:
      targetScreen = DynamicUi::active() ? DynamicUi::screen() : objects.main;
      break;
    case Page::Cells:
